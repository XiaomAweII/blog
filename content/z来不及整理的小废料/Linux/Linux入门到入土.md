...
> 我还记得上亿次学习Linux是我在大二下的时候, 由于我是mac电脑, 我记得很清晰, 我基本上没有办法在机子的电脑上运行虚拟机, 因为我的电脑主存太小, 所以每一次我都很珍惜在机房敲打命令的时间, 当时还立志以后一定要写一个自己的操作系统, 到现在这个idea还没有实现, 不过在不远的将来, 我相信能够自己独立完成一个操作系统的编写, 哈哈哈
> 但是到后来发现还是需要掌握Linux的常用命令, 虽然macos的指令帮助我比别人掌握Linux指令的效率更高, 但是终究还是有所区别, 于是, 趁着这次又租了一台云服务器的时间, 来练习一下Linux指令, 租用的是阿里云最便宜的ECS, 随机用的是CentOS

# Linux 基础

## 操作系统

操作系统 `Operating System`简称OS, 是软件的一部分, 它是硬件基础上的第一层软件, 是硬件和其他软件沟通的桥梁.

操作系统会控制其他程序运行, 管理系统资源, 提供最基本的计算功能, 如管理及配置内存, 决定系统资源供需的优先次序, 同时还提供一些基本的服务程序.
...
- `Linux`发行套件系统使我们常说的`Linux`操作系统, 也即是由`Linux`内核与各种常用软件的集合产品
...
![20240916222519](https://xiaoweii.oss-cn-beijing.aliyuncs.com/img/20240916222519.png)

通过执行`ssh root@你的服务器ip`命令, 然后输入服务器连接密码就可以顺利登录远程服务器. 然后我们就可以在本地电脑操作远程服务器

1. 这个面板就是终端, 也就是`shell`(命令行环境)
2. `ssh root@xxx`这是一条命令, 必须要在`shell`中执行

## Shell

`Shell`这个单词的愿意是"外壳", 跟`kernel`(内核)相对应, 比喻内核外面的一层, 即用户跟内核交互的对话界面

- `Shell`是一个程序, 提供一个与用户对话的环境. 这个环境只有一个命令提示符, 让用户从键盘输入命令, 所以又称为命令行环境(`command line interface`, 简写为`CLI`). `Shell`接收到用户输入的命令, 将命令送入操作系统执行, 并将结果返回用户
- `Shell`是一个命令解释器, 解释用户输入的命令. 它支持变量, 条件判断, 循环操作等语法, 所以用户可以用`Shell`命令写出各种小程序, 又称为`Shell`脚本. 这些脚本都通过`Shell`的解释执行, 而不是通过编译
- `Shell`是一个工具箱, 提供了各种小工具, 供用户方便地使用操作系统的功能.

### Shell的种类

`Shell`又很多种, 只要能给用户提供命令行环境的程序, 都可以看做是`Shell`
...
进入命令行环境以后, 用户会看到`Shell`命令的提示符. 提示符往往是一串前缀, 最后以一个 美元符号`$`结尾, 用户可以在这个符号后面输入各种命令.

执行一个简单的命令`pwd`

```bash
root@iZbp1hf5spam768jowb6ahZ:~# pwd
/root
```
...
- `root` : 表示用户名
- `iZbp1hf5spam768jowb6ahZ` : 表示主机名
- `~` : 表示目前所在目录为根目录, 其中`root`用户的根目录是`/root`, 普通用户的根目录在`/home`下
- `#` : 指示你所具有的权限(`root`用户为`#`,普通用户为`$`)

```bash
root@iZbp1hf5spam768jowb6ahZ:~# whoami
root
root@iZbp1hf5spam768jowb6ahZ:~# hostname
iZbp1hf5spam768jowb6ahZ
```

- 执行`whoami`命令可以查看当前用户名
- 执行`hostname`命令可以查看当前主机名

关于如何创建, 切换, 删除用户, 在后面的用户与权限部分, 这里都先试用`root`用户进行演示

`root`是超级用户, 具备操作系统的一切权限
...
```shell
短参数：command -p 10（例如：ssh root@47.97.85.192 -p 22）
长参数：command --paramters=10（例如：ssh root@47.97.85.192 --port=22）
```
...
├── bin     // 存放基本的二进制可执行文件
├── boot    // 包含Linux启动相关的文件
├── dev     // 设备文件目录，每个文件对应一个硬件设备
├── etc     // 系统配置文件目录
├── home    // 用户的个人目录，类似Windows的"我的文档"
├── lib     // 库文件目录，包含被程序所调用的库文件
├── media   // 可移动设备（如USB驱动器、SD卡）的挂载点
├── mnt     // 临时挂载其他文件系统的目录
├── opt     // 可选应用软件包的安装目录
├── root    // 超级用户root的主目录
├── sbin    // 系统二进制文件目录，包含重要的系统执行文件
├── srv     // 服务数据目录，存放一些网络服务启动后所需的数据
├── tmp     // 临时文件目录，用于存放临时文件
├── usr     // Unix软件资源目录，是最庞大的目录之一，包含大多数用户要调用的程序
└── var     // 可变数据目录，通常包含日志文件等动态内容
```
...
查看命令的可执行性文件所在路径, `Linux`下, 每一条命令其实都对应一个可执行程序, 在终端中输入命令, 按回车的时候, 就是执行了对应的那个程序, `which`命令本身对应的程序也存在于`Linux`中

总的来说一个命令就是一个可执行程序

```bash
root@iZbp1hf5spam768jowb6ahZ:~# which ls
/usr/bin/ls
```
...
```bash
root@iZbp1hf5spam768jowb6ahZ:~# ls -alht
total 64K
dr-xr-x---  9 root root 4.0K Sep  6 12:15 .
-rw-------  1 root root  393 Sep  6 12:15 .mysql_history
-rw-------  1 root root  424 Sep  4 10:17 .bash_history
drwxr-xr-x  3 root root 4.0K Sep  4 09:19 .local
drwxr-xr-x  2 root root 4.0K Sep  4 09:04 workspace
-rw-r--r--  1 root root  203 Sep  4 09:03 .wget-hsts
drwxr-xr-x 21 root root 4.0K Sep  4 09:03 ..
-rw-r--r--  1 root root   33 Sep  4 09:02 readme.txt
drwxr-xr-x  2 root root 4.0K Aug  7 12:00 .rpmdb
drwxr-xr-x  2 root root 4.0K Aug  7 12:00 .pip
-rw-r--r--  1 root root   72 Aug  7 12:00 .pydistutils.cfg
drwx------  2 root root 4.0K Aug  7 11:36 .cache
drwx------  3 root root 4.0K Aug  7 11:35 snap
drwx------  2 root root 4.0K Aug  7 11:35 .ssh
...
`ls -alht`显示所有文件(包括隐藏文件), 使用长列表格式, 以人类可读的方式显示文件大小, 并按修改时间排序(最新的在前)

2. 输出解释

- `total 64K`:当前目录下所有文件总共占用64K空间
- 每行的格式为:权限 链接数 所有者 群组 大小 修改日期 文件名
- 例如: `dr-xr-x--- 9 root root 4.0K Sep 6 12:15 .`
  - `d`:表示这是一个目录
  - `r-xr-x---`:表示权限(所有者可读可执行,群组可读可执行,其他人无权限)
  - `9`:是硬链接数
  - `root root`:分别是所有者和群组
  - `4.0K`:表示文件大小
  - `Sep 6 12:15 `是最后修改时间
...
- `.`:开头的文件是隐藏文件, 如`.mysql_history`,`.bash_history`等
- `..`:表示上级目录

4. 最近修改的文件

- `.mysql_history`(9月6日修改)
- `.bash_history`（9月4日修改）
- `.local 目录`（9月4日修改）
...
- .cache、.pip、.rpmdb：各种缓存和配置目录
- snap：可能与 Snap 包管理系统相关
...
- 根目录 (Root Directory): 是整个文件系统的起点, 包含所有其他目录和文件
- 家目录 (Home Directory): 是用户的个人工作空间, 存储用户的个人文件和配置
...
- `=`键:显示当前页面的内容是文件中的第几行到第几行以及一些其他关于本业内容的详细信息
- `h`键:显示帮助文档
- `/`键:进入搜索模式后, 按`n`键跳到一个符合项目, 按`N`键调到上一个符合项目, 同时也可以输入曾泽表达式匹配
...
**常用参数**: `-p`递归的创建目录结构`mkdir -p one/two/three`
...
`Linux`文件的存储方式分为3个部分, 文件名, 文件内容, 以及权限, 其中文件名的列表是存储在硬盘的其他地方和文件内容是分开存放的, 每个文件名通过`inode`标识绑定到文件内容

Linux下有两种链接类型: 硬链接和短链接
...
![20240917011032](https://xiaoweii.oss-cn-beijing.aliyuncs.com/img/20240917011032.png)
...
![20240917011519](https://xiaoweii.oss-cn-beijing.aliyuncs.com/img/20240917011519.png)
...
![20240917011925](https://xiaoweii.oss-cn-beijing.aliyuncs.com/img/20240917011925.png)
...
常用参数: `-R` 递归设置子目录和子文件， `chown -R lion:lion /home/frank` 把 `frank` 文件夹的用户和群组都改为 `lion`
...
常用参数: `-R` 可以递归地修改文件访问权限，例如 `chmod -R 777 /home/lion`
...
![20240917013749](https://xiaoweii.oss-cn-beijing.aliyuncs.com/img/20240917013749.png)
...
![20240917014020](https://xiaoweii.oss-cn-beijing.aliyuncs.com/img/20240917014020.png)
...
![20240917022633](https://xiaoweii.oss-cn-beijing.aliyuncs.com/img/20240917022633.png)
...
![20240917023111](https://xiaoweii.oss-cn-beijing.aliyuncs.com/img/20240917023111.png)
...
![20240917024203](https://xiaoweii.oss-cn-beijing.aliyuncs.com/img/20240917024203.png)
...
![20240917234016](https://xiaoweii.oss-cn-beijing.aliyuncs.com/img/20240917234016.png)
...
![20240918000615](https://xiaoweii.oss-cn-beijing.aliyuncs.com/img/20240918000615.png)
...
![20240918000926](https://xiaoweii.oss-cn-beijing.aliyuncs.com/img/20240918000926.png)
...
我们来编译安装 htop 软件，首先在它的官网下载源码: https://link.juejin.cn/?target=https%3A%2F%2Fbintray.com%2Fhtop%2Fsource%2Fhtop%23files
...
![20240918002426](https://xiaoweii.oss-cn-beijing.aliyuncs.com/img/20240918002426.png)

修改完服务端配置文件需要重启服务 `systemctl restart sshd`

客户端 `config` 文件的常用配置参数

![20240918002503](https://xiaoweii.oss-cn-beijing.aliyuncs.com/img/20240918002503.png)
...
![20240918005410](https://xiaoweii.oss-cn-beijing.aliyuncs.com/img/20240918005410.png)
